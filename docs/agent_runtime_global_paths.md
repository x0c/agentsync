...
| **DeepSeek Harness（dsh，`@deepseek-ai/dsh`）** | `~/.dsh/AGENTS.md`（官方 `agent-instructions` 插件的全局指令文件，`$DSH_HOME` 默认为 `~/.dsh`；项目发现另读 `AGENTS.md`/`CLAUDE.md`） | `AGENTS.md`（项目根→cwd，发现候选 `AGENTS.md`、`CLAUDE.md`；**注意**：在 `$DSH_HOME` 里开会话时全局与项目 `AGENTS.md` 是同一物理文件，只当全局用） | `~/.dsh/skills/`（另原生扫描通用 `~/.agents/skills/`，agentsync 已收敛） | 配置根 `$DSH_HOME/profiles/<名>/`（profile 清单与 `cordis.patch.yml`）；会话存 `storages/`；密钥 `~/.dsh/.credentials.yaml`（agentsync 不碰） |
