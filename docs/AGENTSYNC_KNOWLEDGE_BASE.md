...
每个 skill 是一个目录，目录内必须包含 `SKILL.md`。脚本、模板、示例、参考文件等与 `SKILL.md` 放在同一 skill 目录下。
...
- 已有工具侧 skill 目录会复制到统一 skill 源，再把工具侧目录替换成软链接。
...
- Codex 的隐藏系统 skill 目录，例如 `.system`，不会被导入或替换。
