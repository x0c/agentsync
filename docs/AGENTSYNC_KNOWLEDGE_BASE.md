...
- 规范目标入口：Codex、OpenCode、Claude Code、Grok 的全局指令文件。
...
| 工具规范入口 | Codex/OpenCode/Claude/Grok 全局指令文件 | 工具读取规范的入口 | 可为 symlink、hardlink 或受管副本 |
...
- `Skills` 区块能报告 `~/.claude/skills`、`~/.codex/skills`、`~/.config/opencode/skill`、`~/.grok/skills`。
